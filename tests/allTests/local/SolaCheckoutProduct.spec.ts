...
/* There are three different versions of the Sola Checkout page.
  * 1. The test PRODUCT page at https://localhost:44532/stonestreet
  * 2. The test DONATION page at http://localhost:44532/charityname
  * 3. The test SUBSCRIPTION page at https://localhost:44532/oldyorktimes
  *
  * The tests will have to updated to be run on different pages.
  */ 
...
test('New Valid CC Purchase Product', async ({ context }) => {
    ...
    await checkoutPage.setOtherAmount(requestAmount);
    await checkoutPage.selectCreditCard();
    await checkoutPage.setCreditCardNum('4444 3333 2222 1111');
...
test('New Valid ACH Purchase Product', async ({ context }) => {
    const extraString = await page.evaluate(() => {
        function generateData() {
            const now = new Date();
            const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${now.getFullYear()}`;
...
test('New Valid Google Pay Purchase Product', async ({ context }) => {
    const extraString = await page.evaluate(() => {
        function generateData() {
            const now = new Date();
            const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${now.getFullYear()}`;
