...
test.beforeAll('open', async ({ browser }) => {
  ...
  await page.goto(authData[environment].baseURL + '/charityname');
...
test('New Valid CC Purchase Donation', async ({ context }) => {
    ...
    await checkoutPage.clickAmount50();
...
test('New Valid ACH Purchase Donation', async ({ context }) => {
    const extraString = await page.evaluate(() => {
        function generateData() {
            const now = new Date();
            const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${now.getFullYear()}`;
...
test('New Valid Google Pay Purchase Donation', async ({ context }) => {
    const extraString = await page.evaluate(() => {
        function generateData() {
            const now = new Date();
            const dateStr = `${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${now.getFullYear()}`;
