...
test('Testing refund of a credit card transaction', async ({ page }) => {
    ...
    await test.step('Find and refund a transaction', async () => {
        await page.reload(); 
        ...
        await page.getByRole('button', { name: 'Refund' }).click();
        await expect(page.getByRole('heading', { name: 'Transaction refunded' })).toBeVisible();
        await page.locator('.modal__close--header').click();
    });
});